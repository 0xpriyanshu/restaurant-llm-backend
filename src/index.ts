...
app.get('/api/restaurants', async (req, res) => {
    try {
      // Check if online filter is present in query
      const { online } = req.query;
      
      // Build query based on online parameter
      const query = online === 'true' ? { isOnline: true } : {};
      
      const restaurants = await Restaurant.find(query);

      // If online filter is true, return simplified data
      if (online === 'true') {
      ...
      // Otherwise return full data as before
      return res.json({
        success: true,
